...
Agent 在 `.agent/memory.md` 中维护一份持久化的项目记忆，跨会话存储和读取知识。

**工作方式：**

1. LLM 在对话中使用 `remember` 工具保存重要发现
2. LLM 使用 `recall` 工具（或 `read_file`）在需要时读取历史知识
3. `/memory` 命令触发 LLM 回顾对话并保存学到的内容
4. System prompt 引导 LLM 主动使用记忆——它会逐渐学会什么值得记住

**存储格式**（人类可读的 markdown）：

```markdown
# Project Memory
- **[architecture]** 2026-05-24 14:30: 使用 WAL 模式 — checkpoint.json + log.jsonl
- **[build]** 2026-05-24 14:31: 运行 `make` 构建，二进制文件在 build/c-agent
- **[convention]** 2026-05-24 14:32: 文件工具必须先调用 resolve_workspace_path
```

**写入路径：**
- LLM 主动调用 `remember` 工具（主要方式）
- `/memory` 命令触发 LLM 总结
- 用户手动编辑 `.agent/memory.md`

**读取路径：**
- LLM 调用 `recall` 工具
- LLM 调用 `read_file` 读取 `.agent/memory.md`
...
- [x] 项目记忆（LLM 驱动的持久化知识存储，.agent/memory.md）
